...
class Defog(nn.Module):
    ...
    def estimateA_batch(self, batch_im, batch_DarkChann):
        ...
        # [n, 1, L]
        dark_vec = batch_DarkChann.view(n, 1, -1)

        # [n, L, c]
        img_vec = batch_im.permute(0, 2, 3, 1).view(n, length, c)

        # 取 top-k 索引: values: [n,1,num], indices: [n,1,num]
        _, indices = dark_vec.topk(k=num, dim=2, largest=True, sorted=False)  # 只要索引

        # 扩展到通道维度，用 gather 抓出对应的像素值
        # indices: [n,1,num] -> [n,num,1] -> broadcast 到 [n,num,c]
        indices_expanded = indices.squeeze(1).unsqueeze(-1).expand(-1, -1, c)  # [n,num,c]

        # gather: [n, L, c] -> [n, num, c]
        top_pixels = torch.gather(img_vec, dim=1, index=indices_expanded)  # [n,num,c]

        # 求平均: [n, c]
        batch_A = top_pixels.mean(dim=1)  # [n,c]
