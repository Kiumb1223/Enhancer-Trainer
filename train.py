...
def main(config:OmegaConf):
    ...
    vis_dir = config.exp.exp_dir + os.sep + 'vis'
    os.makedirs(vis_dir,exist_ok=True) # used for config.enhancer.gdip.middle_vis_dir
    ...
    bt_warmup = False 
    if config.exp.warmup_iter > 0:
        bt_warmup = True
        for p in net.vision_encoder.backbone.parameters():
    ...
    for cur_ep in range(config.exp.max_epoch):
        ...
        for cur_iter,(damage,gt) in enumerate(train_dataloader):
            ...
            if bt_warmup and global_cur_iter > config.exp.warmup_iter:
                for p in net.vision_encoder.backbone.parameters():
            ...
            pred_lst,gate_lst = net(damage)

            # 损失计算
            total_loss , loss_dict = loss(pred_lst,gate_lst,gt,damage)
            ...
            for k , v in loss_dict.items():
                writer.add_scalar(f'Train/{k}',v.item(),global_cur_iter)
        ...
        if (cur_ep + 1) % config.exp.save_model_interval == 0 or (cur_ep + 1) == config.exp.max_epoch:
            ...
            logger.info(f"Model checkpoints saved at {ckpt_dir + os.sep + f'model_epoch_{cur_ep + 1:d}.pth'}.")
        ...
        if (cur_ep + 1) % config.exp.val_epoch == 0:
            ...
            with torch.no_grad():
                for cur_iter,(damage,gt) in enumerate(val_dataloader):
                    ...
                    psnr_ori = calc_psnr(damage,gt).item()
                    ssim_ori = calc_ssim(damage,gt).item()

                    psnr_ori_lst.append(psnr_ori)
                    ssim_ori_lst.append(ssim_ori)
                    ...
                    # 记录实验数据
                    for k,v in loss_dict.items():
                        writer.add_scalar(f'Eval/{k}',v.item(),global_cur_iter)
                            
                    writer.add_scalar('Eval/psnr[ori]',psnr_ori,global_cur_iter)
                    writer.add_scalar('Eval/ssim[ori]',ssim_ori,global_cur_iter)

                    writer.add_scalar('Eval/psnr[aug]',psnr,global_cur_iter)
                    writer.add_scalar('Eval/ssim[aug]',ssim,global_cur_iter)
                
                val_epoch_cnt += 1

            logger.info(f'Evalation:[{cur_ep}], Mean Loss:[{np.mean(loss_lst):.3f}], PSNR:[{np.mean(psnr_lst):.3f}/{np.mean(psnr_ori_lst):.3f}], SSIM:[{np.mean(ssim_lst):.3f}/{np.mean(ssim_ori_lst):.3f}].')
